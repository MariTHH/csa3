...
* Память данных: адресуется через регистр data_address; может быть записана: из аккумулятора acc; с буферного регистра br; может быть прочитана в аккумулятор acc.
* Регистр аккумулятора acc: может быть подан на вывод; используется как флаг (сравнение с 0).
